    ...
    if ($resultGPA){
        while ($resultGPARow = mysqli_fetch_assoc($resultGPA)) {
            ...
            if ($gpa>$gpa1 && $gpa<$gpa2){
                $eids[]=$resultGPARow['enrollment_id'];
            }
        }
    }
    $eidids = implode(',', array_values($eids));
    $sql = "select student.student_id,grade,student.name,program.program_name from student left join enrollment on enrollment.student_id = student.student_id left join program on program.program_id = student.program_id where enrollment.enrollment_id in ($eidids)";
    ...
    if ($result2) {
        while ($resultRow = mysqli_fetch_assoc($result2)) {
            $rows[] = [
                ...
                'department_id'=>$resultRow['department_name'],
                'enrollment_count'=>$resultRow['count'],
            ];
        }
    }
    $res = [];
